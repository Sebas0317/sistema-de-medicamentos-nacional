  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} min={new Date(Date.now() + 86400000).toISOString().split('T')[0]} className={`w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-accent outline-none ${errors.fecha ? 'border-red-300' : 'border-gray-200'}`} />
        {errors.fecha && <p className="text-xs text-red-500 mt-1">{errors.fecha}</p>}
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Hora</label>
        <select value={hora} onChange={(e) => setHora(e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-accent outline-none">
      ...
      <div className="flex items-center gap-2">
        <input type="checkbox" id="domicilio" checked={esDomicilio} onChange={(e) => setEsDomicilio(e.target.checked)} className="rounded border-gray-300 text-accent" />
        <label htmlFor="domicilio" className="text-sm text-gray-700">A domicilio</label>
      </div>
      {esDomicilio && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Dirección</label>
          <input type="text" value={direccion} onChange={(e) => setDireccion(e.target.value)} className={`w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-accent outline-none ${errors.direccion ? 'border-red-300' : 'border-gray-200'}`} />
      ...
      <div className="flex gap-3">
        ...
        <button type="submit" disabled={isLoading} className="flex-1 py-2 text-sm font-medium text-white bg-accent hover:opacity-90 rounded-lg disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
